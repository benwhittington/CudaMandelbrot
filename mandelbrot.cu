...
void mb() {
    ...
    constexpr size_t density = 28;
    constexpr size_t pixelsPerX = 7 * density;
    constexpr size_t pixelsPerY = 3 * density;

    auto domain = Domain<double>(minX, maxX, minY, maxY);
    const auto screen = Screen(domain, pixelsPerX, pixelsPerY);

    auto runner = RunAndRenderMandelbrotDeviceRaii<double>(screen.m_numPixels);

    std::vector<char> charsOut(screen.m_numPixels);
    ...
    while (true) {
        ...
        runner(domain, screen, charsOut.data());
        PrintChars(screen, charsOut.data());
